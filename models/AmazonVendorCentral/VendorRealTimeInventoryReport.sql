{% if var("VendorRealTimeInventoryReport") %} {{ config(enabled=True) }}
...
{% set result =set_table_name("VendorRealTimeInventoryReport_tbl_ptrn","VendorRealTimeInventoryReport_exclude_tbl_ptrn") %}
...
{% for i in result %}
        ...
        {% if is_incremental() %}
            ...
            WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('VendorRealTimeInventoryReport_lookback') }},0) from {{ this }})
        {% endif %}
