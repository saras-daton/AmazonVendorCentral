{% if var("VendorTrafficReport") %} 
    {{ config( 
        enabled = True,
        post_hook = "drop table {{this|replace('VendorTrafficReport', 'VendorTrafficReport_temp')}}"
    ) }}
{% else %}
...
{% for i in results_list %}
    ...
    {%- set query -%}
    {{action1}}
    {{tbl|replace('VendorTrafficReport', 'VendorTrafficReport_temp')}}
...
{% endfor %}
select * from {{this|replace('VendorTrafficReport', 'VendorTrafficReport_temp')}}    
