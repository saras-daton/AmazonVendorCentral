{% if var("VendorRealTimeSalesReport") %} {{ config(enabled=True) }}
...
{% set result =set_table_name("VendorRealTimeSalesReport_tbl_ptrn","VendorRealTimeSalesReport_exclude_tbl_ptrn") %}
...
{% for i in result %}
        ...
        {% if is_incremental() %}
            ...
            WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('VendorRealTimeSalesReport_lookback') }},0) from {{ this }})
        {% endif %}
