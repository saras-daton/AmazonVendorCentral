...
{% for i in result %}
        ...
        shippedUnits,
        {{ currency_conversion('b.value','b.from_currency_code','shippedCogs.currencyCode') }},
        ...
        {% if var('currency_conversion_flag') %}
            left join {{ref('ExchangeRates')}} b on date(startdate) = b.date and shippedCogs.currencyCode = b.to_currency_code
        {% endif %}
        {% if is_incremental() %}
            {# /* -- this filter will only be applied on an incremental run */ #}
            WHERE a.{{daton_batch_runtime()}}  >= (select coalesce(max(_daton_batch_runtime) - {{ var('VendorSalesReportBySourcing_lookback') }},0) from {{ this }})
        {% endif %}
