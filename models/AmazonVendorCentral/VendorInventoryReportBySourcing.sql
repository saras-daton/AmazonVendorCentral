{% if var("VendorInventoryReportBySourcing") %}
    {{ config( 
        enabled = True,
        post_hook = "drop table {{this|replace('VendorInventoryReportBySourcing', 'VendorInventoryReportBySourcing_temp')}}"
    ) }}
{% else %}
...
{% for i in results_list %}
    ...
    {%- set query -%}
    {{action1}}
    {{tbl|replace('VendorInventoryReportBySourcing', 'VendorInventoryReportBySourcing_temp')}}
...
{% endfor %}
select * from {{this|replace('VendorInventoryReportBySourcing', 'VendorInventoryReportBySourcing_temp')}}    
